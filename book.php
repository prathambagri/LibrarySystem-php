    ...
    <div class="row">
        ...
        <div class="col-md-8 blog-body">
            ...
            foreach ($cur as $result) {
                $mydb->setQuery("SELECT review FROM `tbltransaction` WHERE IBSN = " . $result->IBSN);
                $usesbooks_reviews = $mydb->loadResultlist();

                $sum_reviews = 0;
                $count_reviews = 0;

                foreach ($usesbooks_reviews as $b) {
                    if ($b->review !== null) {
                        $sum_reviews += $b->review;
                        $count_reviews += 5;
                    }
                }

                $average_review = ($count_reviews > 0) ? $sum_reviews / $count_reviews * 5 : 0;
                $full_stars = floor($average_review);
                $half_star = ($average_review - $full_stars >= 0.5) ? 1 : 0;
                $empty_stars = 5 - $full_stars - $half_star;
                $star_rating = str_repeat('★', $full_stars) . str_repeat('☆', $half_star) . str_repeat('☆', $empty_stars);
            ?>
                <!-- Start Blog post -->
                <div class="blog-post">
                    <h1 class="post-title ggg">
                        <a href="index.php?q=bookdetails&id=<?php echo $result->IBSN; ?>">
                            <?php echo $result->BookTitle; ?>
                        </a>
                    ...
                    <ul class="pagination">
                        ...
                        <li class="page-item <?php if ($page <= 1) {
                            ...
                            <a class="page-link" href="<?php if ($page <= 1) {
                                                            ...
                                                            echo "index.php?q=books&category=" . $category . "&page=" . ($page - 1);
                                                        } ?>">Prev</a>
                        </li>
